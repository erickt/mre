...
pub fn response(m2: mongrel2::Connection, req: @mongrel2::Request) -> @response {
    @{
        ...
        mut end: || { m2.reply(req, str::to_bytes("")) },
    }
}
...
impl @response {
    ...
    fn set_header(+name: str, +value: str) {
        let mut values = match (*self.headers).find(name) {
          None => {
            let values = @DVec();
    ...
    fn reply_head() {
        let mut rep = DVec();
        rep.push_all(str::to_bytes(#fmt("HTTP/1.1 %u %s\r\n",
                                     self.code,
                                     *self.status)));
    ...
    fn reply_http<T: ToBytes>(code: uint, body: T) {
    ...
    fn reply_text<T: ToBytes>(code: uint, body: T) {
        self.set_content_type("text/plain");
        self.reply_http(code, body)
    }

    fn reply_html<T: ToBytes>(code: uint, body: T) {
        self.set_content_type("text/html");
        self.reply_http(code, body)
    }

    fn reply_json<T: ToJson>(code: uint, body: T) {
